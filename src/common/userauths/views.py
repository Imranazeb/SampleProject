...
def register(request):
    ...
    if request.method == "POST":
        ...
        if len(email) < 5 or len(email) > 254:
            ...
            messages.error(
                request,
                "Password must contain at least 8 characters with upper case, lower case, alpha-numeric, and special characters",
            )
            return render(request, "userauths/register.html", context)
            ...
            User.objects.create_user(
                username=username, email=email, full_name=full_name, password=password1
            )
